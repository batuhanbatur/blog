"use client"

import { useState, useEffect } from "react"

const linkStyle = {
  display: "flex",
  alignItems: "center",
  gap: "8px",
  fontSize: "13px",
  color: "#1D1D0C",
  textDecoration: "none",
  border: "1px solid rgba(29, 29, 12, 0.15)",
  borderRadius: "4px",
  padding: "8px 16px",
  opacity: 0.7,
}

const tagStyle = {
  display: "flex",
  alignItems: "center",
  gap: "8px",
  fontSize: "13px",
  color: "#1D1D0C",
  border: "1px solid rgba(29, 29, 12, 0.15)",
  borderRadius: "4px",
  padding: "8px 16px",
  opacity: 0.7,
}

const contactStyle = {
  fontSize: "12px",
  letterSpacing: "0.1em",
  textTransform: "uppercase",
  color: "#1D1D0C",
  textDecoration: "none",
  border: "1px solid rgba(29, 29, 12, 0.3)",
  borderRadius: "4px",
  padding: "8px 20px",
}

export default function AboutPage() {
  const [isMobile, setIsMobile] = useState(false)

  useEffect(() => {
    const check = () => setIsMobile(window.innerWidth < 768)
    check()
    window.addEventListener("resize", check)
    return () => window.removeEventListener("resize", check)
  }, [])

  const content = (
    <>
      <div
        style={{
          fontSize: isMobile ? "16px" : "18px",
          lineHeight: "1.8",
          color: "#1D1D0C",
        }}
      >
        <p style={{ margin: "0 0 24px 0", opacity: 0.7 }}>
          There are probably more words to describe me. These are just the first
          ones that come to mind.
        </p>
        <p style={{ margin: "0 0 24px 0", opacity: 0.7 }}>
          This blog is my first real step into a new world. Here, I experiment
          with different blog concepts while also treating this place a bit like
          a personal social platform. The project is still evolving. So am I.
        </p>
      </div>

      <div style={{ marginTop: "48px", marginBottom: "48px" }}>
        <p
          style={{
            fontSize: "16px",
            color: "#1D1D0C",
            opacity: 0.7,
            margin: "0 0 16px 0",
            lineHeight: "1.7",
          }}
        >
          New gaming buddies? Always open to that. Feel free to add me on Steam,
          League of Legends, or Battle.net.
        </p>

        <div
          style={{
            display: "flex",
            gap: "12px",
            flexWrap: "wrap",
            marginBottom: "32px",
          }}
        >
          <a
            href="https://steamcommunity.com/profiles/76561198046414837/"
            target="_blank"
            rel="noopener noreferrer"
            style={linkStyle}
          >
            <img
              src="https://cdn.simpleicons.org/steam/1D1D0C"
              width="14"
              height="14"
              alt="Steam"
            />
            Steam
          </a>
          <div style={tagStyle}>
            <img
              src="https://cdn.simpleicons.org/riotgames/1D1D0C"
              width="14"
              height="14"
              alt="League"
            />
            {"Crüel Sun #7oo7"}
          </div>
          <div style={tagStyle}>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 640 640"
              width="14"
              height="14"
              fill="#1D1D0C"
            >
              <path d="M512.8 289.6C539.7 289.8 548.4 282.2 551.7 277.2C564.2 260.9 544.6 229.6 498.9 205.9C516.7 172.3 529 142.2 535.2 120.6C538.6 108.8 536.3 101.6 535.7 100.3C534 110.8 519.9 148.8 487.5 200.3C462.5 189.1 431 180.2 393.7 176.5C384.8 159.6 358.8 112.6 333.2 87.6C316.4 71.1 302.9 65 292.4 64.1L292.4 64.1C278.6 62.5 269.7 69.9 265 75.1C247.8 93.6 240.7 124 240 159.2C232.8 146.9 222.8 134.6 211.5 133.3L211.3 133.3C190.6 129.8 172.9 162.5 175.3 214.6C136.9 216 104.3 220.4 82.3 225.8C72.4 228.2 66.1 233.1 64.5 235.5C65.5 235.1 86.9 226.3 176.1 226.3C181.3 279.3 205.9 328.1 202.1 319.5C192.4 334.9 163.9 381.9 154.8 417.2C148.9 440.1 150.4 454.8 155 464.3C160.6 477.1 171.4 481 178.2 482.6C203.2 488.3 233.6 479 264.9 461.5C257.4 474.3 251 490 255.8 500.8C263.1 520.4 300.3 519.5 344.2 491.4C364.4 523.6 384.3 549.3 399.9 565.5C402.5 568.3 405.4 570.7 408.7 572.6C413.8 575.8 417.3 576 417.3 576C409.1 569.3 383.3 538 354.8 484.2C377 468.2 400.5 445.3 422.3 414.9C545.1 419.5 565.6 390.1 570.3 383.3C585 363.4 573.7 325.9 513 289.6zM435 395.8C458.8 358.1 465.3 328 464.4 303.5C492.3 321.1 511.6 341.1 513.5 362.3C514.6 375.2 505.4 391.4 435 395.8zM281.1 451.7C290.9 445.5 300.6 438.6 310.3 431.2C317 444.5 323.9 457.3 330.9 469.4C290.3 491.3 262.1 482.2 281.1 451.7zM496.1 280.4C485.8 275.1 474.9 270.1 463.7 265.4C471.6 253.3 479.2 241 486.4 228.5C525.5 252.6 532.3 281.7 496 280.4zM343.4 462C337.9 450.6 332.4 438.5 326.9 425.6C370.1 426.9 389.3 406.9 390.2 405.2C390.2 405.3 365.2 420.8 327.7 417.4C358.3 391.8 386.8 363.7 412.8 333.4C421.5 323.2 429.8 312.8 437.7 302.3C437.3 302 436.2 299.3 421.2 290.3C369.5 350.6 318.9 388.3 288.4 406.2C267.8 395 247.6 374.4 232.7 344.7C212.7 304.8 202.7 262.3 201.1 228.6C213.4 229.5 226.4 230.8 239.9 232.5C217.6 269.3 225.5 295.5 226.4 296.7C226.4 296.6 225.4 267.5 246.5 237.1C255.5 289.6 270.5 340.9 291.2 389.9C292.1 389.5 293 390.8 309.9 381.7C283.6 307.2 276.1 243.5 275.9 208.3C295.9 195.9 324.1 188.5 357.5 190.5C402.1 193.2 443.9 205.7 473.8 221.2C466.7 231.6 458.9 242.5 450.5 253.7C429.8 216 403.2 209.8 401.6 210C401.7 210 427.5 224.1 443.1 257.2C405.6 243.4 367 232.8 327.8 225.5C314.7 223.1 301.6 221.1 288.4 219.5C288.3 219.9 286.6 221.3 286.2 239.8C364.1 254.3 422.8 279.7 453.4 297C454.1 320.6 446.4 348.6 428 376.6C403.4 413.9 371.6 443.8 343.2 462zM370.8 175C326.2 173.3 297.2 182.4 276.1 195.7C278.1 143.4 297.4 119.3 314.3 120.4C331.2 116.2 369.2 172.6 370.8 175zM240.1 178.1C240.6 190.2 241.7 202.7 243.3 215.4C228.7 214.5 214.6 214.1 200.9 214.1C200.8 217.3 200.8 163.1 225.6 164.5L225.7 164.5C231.5 165.6 236.3 171.4 240.1 178.1zM212 340.1C232.8 379.8 255.3 400.7 277.3 412.4C230.5 437.2 199.8 432.4 192.4 416.9C192.2 416.7 181.3 401.6 212.1 340.1L212.1 340.1z" />
            </svg>
            {"Avare#21640"}
          </div>
        </div>

        <p
          style={{
            fontSize: "16px",
            color: "#1D1D0C",
            opacity: 0.7,
            margin: "0 0 16px 0",
            lineHeight: "1.7",
          }}
        >
          Curious about my anime taste? Check out my MyAnimeList profile:
        </p>

        <a
          href="https://myanimelist.net/profile/batuhanbatur"
          target="_blank"
          rel="noopener noreferrer"
          style={{ ...linkStyle, display: "inline-flex" }}
        >
          <img
            src="https://cdn.simpleicons.org/myanimelist/1D1D0C"
            width="14"
            height="14"
            alt="MAL"
          />
          MyAnimeList
        </a>
      </div>

      <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
        <p
          style={{
            fontSize: "14px",
            color: "#1D1D0C",
            opacity: 0.5,
            margin: 0,
          }}
        >
          Feel free to reach out.
        </p>
        <a href="/contact" style={contactStyle}>
          Contact
        </a>
      </div>
    </>
  )

  if (isMobile) {
    return (
      <main style={{ padding: "48px 24px", fontFamily: "Satoshi, sans-serif" }}>
        <h1
          style={{
            fontSize: "clamp(52px, 14vw, 68px)",
            fontWeight: "800",
            fontFamily: "Tanker, sans-serif",
            color: "#1D1D0C",
            lineHeight: "1.0",
            margin: "0 0 32px 0",
          }}
        >
          DEVELOPER. <br />
          GAMER. <br />
          RUNNER. <br />
          ANIME LOVER.
        </h1>

        <div
          style={{
            width: "100%",
            maxHeight: "480px",
            marginBottom: "40px",
            borderRadius: "4px",
            overflow: "hidden",
          }}
        >
          <img
            src="/about.jpg"
            alt="Batuhan Batur"
            style={{
              width: "100%",
              height: "100%",
              display: "block",
              objectFit: "cover",
              objectPosition: "top",
            }}
          />
        </div>

        {content}
      </main>
    )
  }

  return (
    <main
      style={{
        maxWidth: "900px",
        margin: "0 auto",
        padding: "80px 24px",
        fontFamily: "Satoshi, sans-serif",
      }}
    >
      <div style={{ display: "flex", alignItems: "flex-start", gap: "64px" }}>
        <div style={{ flex: 1 }}>
          <h1
            style={{
              fontSize: "48px",
              fontWeight: "800",
              fontFamily: "Tanker, sans-serif",
              color: "#1D1D0C",
              lineHeight: "1.0",
              margin: "0 0 48px 0",
            }}
          >
            DEVELOPER. GAMER.
            <br />
            RUNNER. ANIME LOVER.
          </h1>
          {content}
        </div>

        <div
          style={{
            width: "300px",
            flexShrink: 0,
            overflow: "hidden",
            borderRadius: "4px",
          }}
        >
          <img
            src="/about.jpg"
            alt="Batuhan Batur"
            style={{ width: "100%", display: "block", objectFit: "cover" }}
          />
        </div>
      </div>
    </main>
  )
}
